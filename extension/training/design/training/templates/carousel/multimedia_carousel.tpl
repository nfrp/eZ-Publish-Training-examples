...
YUI( YUI3_config ).use( 'node', 'event', 'io-ez', function(Y, result) {ldelim}
    ...
    Y.on('domready', function(e) {ldelim}
        ...
        var offset = 0;
        var limit = 3;
        var total = {$params.pictures|count()};
        ...
        var handleRequest = function(e) {ldelim}
            ...
            var colContent = Y.Node.all('#block-{$params.id} .col-content');
            ...
            data += '&limit=' + limit;
            data += '&gallery_id={$params.id}';
            
            Y.io.ez( 'training::getnextitems', {ldelim} on: {ldelim} success: _callBack {rdelim}, method: 'POST', data: data {rdelim} );
        {rdelim};
        ...
        {rdelim};
            ...
            var prevButton = Y.one('#block-{$params.id} input.carousel-prev-button');
            prevButton.on('click', handleRequest);

            var nextButton = Y.one('#block-{$params.id} input.carousel-next-button');
...
{def $pictures = $params.pictures}
<!-- BLOCK: START -->
<div id="block-{$params.id}" class="block-type-gallery">
...
{node_view_gui view='block_item' image_class='blockgallery1' content_node=$pictures[0]}
...
{node_view_gui view='block_item' image_class='blockgallery1' content_node=$pictures[1]}
...
{node_view_gui view='block_item' image_class='blockgallery1' content_node=$pictures[2]}
